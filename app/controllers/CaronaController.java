...
public class CaronaController extends Controller {
    ...
    public static Result createCarona(){
        ...
        String actualUserRegistration = verificaToken(token);
        if(actualUserRegistration == null){
            return showNewCarona(token, "Token Invalid", true);
        }else {
            //User user = db.searchUserByRegistration(actualUserRegistration);
            ...
            User user = getUser();
            Carona carona = new Carona(user.getEmail(), startingDistrict, startingRoad, arrivalDistrict, arrivalRoad, startingTme, Integer.parseInt(numberVacancies));
